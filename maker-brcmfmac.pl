...
use strict;
	...
	if (not defined $pahole or
	    ...
	    not defined $cmdName) {
		die "Usage:\n\tmaker-brcmfmac.pl <pahole> <struct> <cmd> [prefix]\n";
	}
	...
	# Grab command nr
	my $cmdTmp = `cat fwil.h | grep $cmdName`;
	$cmdTmp =~ /(\d+)/;
	my $cmdNr = $1;
	...
	foreach $line (@lines) {
		...
		if ($size == 6 && $name eq "bssid") {
			$d .= "f.$fn = ProtoField.ether(\"bcm_cdc_ioctl.$fn\", \"$name\")\n";
		} elsif ($size == 6 && $name eq "ea") {
			$d .= "f.$fn = ProtoField.ether(\"bcm_cdc_ioctl.$fn\", \"$name\")\n";
		} elsif ($size == 1) {
			$d .= "f.$fn = ProtoField.uint8(\"bcm_cdc_ioctl.$fn\", \"$name\")\n";
		} elsif ($size == 2) {
			$d .= "f.$fn = ProtoField.uint16(\"bcm_cdc_ioctl.$fn\", \"$name\")\n";
		} elsif ($size == 4) {
			$d .= "f.$fn = ProtoField.uint32(\"bcm_cdc_ioctl.$fn\", \"$name\")\n";
		} else {
			$d .= "f.$fn = ProtoField.bytes(\"bcm_cdc_ioctl.$fn\", \"$name\")\n";
		}
	}
